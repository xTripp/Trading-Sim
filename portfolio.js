...
class Portfolio {
    ...
    buy(symbol, count, price) {
        ...
        if (parseFloat(cost) > this.balance) {
            return JSON.stringify({type: 'info', val: `Insufficient funds. You are buying ${count} $${symbol} shares for $${cost}, but you only have $${this.balance}`});
        }
        ...
        return JSON.stringify({type: 'info', val: `$${symbol} buy @ ${parseFloat(price.toFixed(2))}. New balance: ${this.balance}`});
    }

    sell(symbol, count, price) {
        const total = this.portfolio.reduce((total, asset) => asset.symbol === symbol ? total + asset.quantity : total, 0);
        if (total < count) {
            return JSON.stringify({type: 'info', val: `Insufficient shares. You are selling ${count} $${symbol} shares but you have ${total}`});
        }
        ...
        return JSON.stringify({type: 'info', val: `$${symbol} sell @ ${parseFloat(price.toFixed(2))}. New balance: ${this.balance}`});
    }
}
